...
class Addition(Node): # TO-DO: OPTIMIZE THIS MORE
    ...
    def set(self, inputA:Node, inputB:Node):
        ...
        if type(inputA) == Number and type(inputB) == Number:
            ...
            self.output = Coordinate(Number(inputA.__x+inputB.__x), Number(inputA.__y+inputB.__y))
        elif type(inputA) == Frame and type(inputB) == Time:
            self.output = Frame(inputA.__value, Addition(inputA.__time, inputB))
        elif type(inputA) == Frame and inputA.type == type(inputB):
            self.output = Frame(Addition(inputA.__value, inputB), inputA.__time)
    ...
    def update(self):
        if type(self.inputA) == Number and type(self.inputB) == Number:
            ...
            self.output.set(Number(self.inputA.__x+self.inputB.__x), Number(self.inputA.__y+self.inputB.__y))
        elif type(self.inputA) == Frame and type(self.inputB) == Time:
            self.output.set(self.inputA.__value, Addition(self.inputA.__time, self.inputB))
        elif type(self.inputA) == Frame and self.inputA.type == type(self.inputB):
            self.output.set(Addition(self.inputA.__value, self.inputB), self.inputA.__time)
